                                    ...
                                    <tbody>
                                        <?php foreach ($services as $service) : ?>
                                            <tr>
                                                ...
                                                <a href="<?= base_url('service-detail/').$service['cacheKey'] ?>" target="_blank">
                                                <span class="font-semibold"><?= $service['service_name'] ?></span></a>
                                                </td>
                                                <td class="text-start border-t border-gray-100 dark:border-gray-800 p-4">
...
<script>
    ...
    $(document).ready(function() {
        $('.servicebtn').on('click', function() {
            console.log('clicked');
            var cacheKey = $(this).data('cachekey');console.log(cacheKey);
            $.post('<?= base_url('service-detail') ?>', {
                cacheKey: cacheKey
            }, function(data) {
                $('#serviceDetailModal .modal-body').html(data);
            });
        });
    });
